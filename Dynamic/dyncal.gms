...
parameter 		elecout_t(t,sub_elec)     trend for electricity generation trend
                bata(sub_elec)   		  learning coefficient
;
                bata(sub_elec) 		= 	0;

*// from research of Ding Qun                
                bata("wind")  		= 	-0.1265928;
                bata("solar")  		= 	-0.198229;
*// to update
...
*==========save benchmark data ===========
				...
				fact_supp("capital","2012")		
									= 	fact("capital") ;
				fact_supp("labor","2012")
									= 	tqlabor_s0 ;
				AEEI_t(s,"2012") 	=	aeei(s);
				xscale_t("2012") 	= 	xscale;
				mkup_t("2012",sub_elec) 
									=	emkup(sub_elec);
				sub_t("2012",sub_elec) 
									=  	t_re.l(sub_elec);
				price_co2_t(t) 		=	0;
				elecout_t("2012",sub_elec)
									=	qelec.l(sub_elec);

				rk0$((gammak+deltak) le zetak)
									= 	(gammak+deltak)*fact("capital")/invest_k("2012")-deltak;
				rk0$((gammak+deltak) > zetak) 
									=	(betak/2*(gammak+deltak-zetak)**2+gammak+deltak)*fact("capital")/invest_k("2012")-deltak;
				rork0				= 	rk0+deltak;
...
parameter 	clim_scn 		"switch for national carbon market 0=off,1=accounting; 2= quantity target;3 = intensity target"
			clim_ms_scn		"switch for sectoral carbon market 0=off,1=exogenous carbon price; 2= quantity target;3 = intensity target;"
			clim_m_scn 		"switch for sector in the sectoral carbon market 0=not in, 1 = in;"
			clim_trend_scn
			tax_scn
			renewable_scn
			ecf_scn;
			...
			renewable_scn(t,sub_elec,"BAU")
									=	RET1(t,sub_elec);
			renewable_scn(t,sub_elec,pscn)
									=	RET2(t,sub_elec);
